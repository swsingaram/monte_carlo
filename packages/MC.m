...
MCEnsembleOneD::usage = "MCEnsembleOneD[m,n,no,trials,step,e] runs the Metropolis Algorithm on 1D lattices. m -- number of 1D lattices; n -- number of sites; no -- number of proteins in the system (no/(m*n) is the CP:RNA ratio); trials -- total number of MC steps in simulation; step -- frequency for which configurations are sampled (saved); e -- lateral energy"
...
RgGraph::usage = "Returns Rg for a tree graph based on Kramer's formula.  Input is an adjacency matrix."


Begin["`Private`"]

MCEnsembleOneD[m_,n_,no_,trials_,step_,e_] := (OccupiedSites = {};
While[Less[Length[OccupiedSites], no], (*WHILE 1*)
	OccupiedSites = Union[OccupiedSites,DeleteDuplicates[RandomSample[Range[Times[n,m]], no]]];
	UnOccupiedSites = Complement[Range[Times[n,m]], OccupiedSites];
	Config = ConstantArray[0,Times[n,m]];
	Energy = 0;
	Do[Config[[OccupiedSites[[i]]]] = 1; (*DO 1*)
	
		If[And[Unequal[Mod[OccupiedSites[[i]],n],1], Equal[Config[[OccupiedSites[[i]] - 1]], 1]],
			Energy = Energy - e;];
	,{i,1,no}]; (*END DO 1*)
	
	Do[TempConfig = Config; (*DO 2*)
	TempOcc = OccupiedSites;  
	TempUnOcc = UnOccupiedSites; 
	randI1 = RandomInteger[{1,no}];  
	randOcc = OccupiedSites[[randI1]];  
	randI2 = RandomInteger[{1, Times[m,n] - no}];  
	randUnOcc = UnOccupiedSites[[randI2]];
      	TempConfig[[randOcc]] = 0;
      	TempConfig[[randUnOcc]] = 1;
       	TempOcc[[randI1]] = randUnOcc;
      	TempUnOcc[[randI2]] = randOcc;
      	Dele = 0;  nni = 0;  nnf = 0;
    		If[And[Unequal[Mod[randOcc,n],0],Equal[Config[[randOcc + 1]], 1]], 
	        	nni = nni + 1;, 
		       	nni = nni;  ];    
			
		If[And[Unequal[Mod[randOcc,n],1],Equal[Config[[randOcc - 1]], 1]],
	  		nni = nni + 1;,
		  	nni = nni;      ];
		    
		If[And[Unequal[Mod[randUnOcc,n],0],Equal[TempConfig[[randUnOcc + 1]], 1]],
	  		nnf = nnf + 1;,
		  	nnf = nnf;     ];
		     
		If[And[Unequal[Mod[randUnOcc,n],1],Equal[TempConfig[[randUnOcc - 1]], 1]],
	      		nnf = nnf + 1;,
		      	nnf = nnf;  ];
			
		Delnn = nnf - nni;
		Dele = Times[Delnn,Times[-1,e]];
	      
		If[LessEqual[Dele,0],
	    		Energy = Energy + Dele;
		     	Config = TempConfig;
		    	OccupiedSites = TempOcc;
		    	UnOccupiedSites = TempUnOcc;,
		    
			r = RandomReal[];
		    	If[Less[r,Exp[Times[-1,Dele]]],
		 		Energy = Energy + Dele;
			 	Config = TempConfig;
			 	OccupiedSites = TempOcc;
			 	UnOccupiedSites = TempUnOcc;,
		...
		If[Equal[Mod[numtrials,step],0],
	       (*DO 3*)
			Do[    WriteConfig = Part[Config,((i - 1)*n + 1);;(i*n)];
		       		NumProt = Count[WriteConfig,1]; 
				PutAppend[WriteConfig,StringJoin[ToString[Divide[no,m]],"configurations.dat"]];
			       	PutAppend[NumProt,StringJoin[ToString[Divide[no,m]],"numprotein.dat"]];
			       	,{i,1,m}];(*END DO 3*)

			PutAppend[Energy,StringJoin[ToString[Divide[no,m]],"energy.dat"]];
			       
		];
	,{numtrials,1,trials}];(*END DO 2*)
...
ClearAll[product,graph,brokengraph,listofedges,conncomps];
